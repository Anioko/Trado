...
def events_list():
    appts = Event.query.filter(Event).filter(Event.end_date >= datetime.now()).order_by(Event.pub_date.asc()).all()
...
def event_details(event_id, event_title, event_city, event_state, event_country):
    ...
    return render_template('events/event_details.html', appt=appts, orgs=orgs, users=users)
